...
```{r}
scot_accom_raw %>%
  filter(date_code == "2019" & location == "Large Urban Areas") %>%
...
```{r}
scot_accom_raw %>%
  filter(date_code == "2019" & location == "Remote Rural") %>%
  group_by(accommodation_type_and_occupancy) %>%
  summarise(occupancy = value) %>%
  radarchart::chartJSRadar() 
```
...
```{r}
scot_accom_raw %>%
  filter(date_code == "2013" & location == "Large Urban Areas") %>%
  group_by(accommodation_type_and_occupancy) %>%
  summarise(occupancy = value) %>%
  radarchart::chartJSRadar() 
```
...
```{r}
scot_accom_raw %>%
  filter(date_code == "2013" & location == "Remote Rural") %>%
  group_by(accommodation_type_and_occupancy) %>%
  summarise(occupancy = value) %>%
  radarchart::chartJSRadar() 
```
